...
async fn parts_and_segments_follow_the_config() {
    ...
    for tag in ["#EXTM3U", "#EXT-X-VERSION:9", "#EXT-X-TARGETDURATION:2", "#EXT-X-MAP:URI=\"init.mp4\"", "#EXT-X-MEDIA-SEQUENCE:0"] {
    ...
    let extinf: Vec<f64> = tag_values(&pl, "#EXTINF:").iter().map(|v| v.trim_end_matches(',').parse().unwrap()).collect();
    ...
    for &(msn, i, d, ind) in &ps {
        ...
        assert!(d >= 0.85 * 0.2 || ps.iter().all(|&(m, j, ..)| m != msn || j <= i), "short part s{msn}.p{i} ({d} s) is not last");
...
async fn lagging_packager_restarts_on_a_keyframe_with_a_discontinuity() {
    ...
    assert!(after.lines().find(|l| l.starts_with("#EXT-X-PART:")).is_none_or(|l| l.contains("INDEPENDENT=YES")), "{pl}");
    drop(p);
}
