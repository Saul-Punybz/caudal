...
**Last updated:** 19 Sep 2026, ~01:35 UTC (PR #1 open: crash safety, honest CI, RTMP wrap, CUE-OUT; benchmark running; see RESUME HERE)
...
**Benchmark vs MediaMTX (Opus agent) may still be running** in worktree `.claude/worktrees/agent-a8c46b9e873ee0eaf` (branch `worktree-agent-a8c46b9e873ee0eaf`). Its report went to the old session. Check `pgrep -fl bench/run.sh`; when it has stopped, read `bench/` and `docs/research/BENCH-MEDIAMTX.md` in that worktree, verify the numbers make sense, commit/merge them, then `git worktree remove` it. **Do not compile locally while it runs** (it skews CPU numbers).
**Benchmark vs MediaMTX v1.21.0, first full pass (Apple M4, 10 cores; 3 repetitions; raw file `bench/results/20260918-202735.md` in the benchmark worktree; the agent was still running a second pass at 01:51 UTC):** binary 21.8 vs 54.1 MB; idle RSS 14 vs 35 MB; 1 publisher RSS 98 vs 80 MB (Caudal heavier: 50 s buffer). **LL-HLS fan-out, Caudal wins:** CPU 8.7 vs 26.7 % (100 viewers), 26.5 vs 72.4 % (300), 83 vs 168 % (1,000), RSS 215 vs 339 MB at 1,000, all viewers kept up on both. **RTSP:** Caudal slightly more CPU (122 vs 104 % at 100); at 1,000 both collapse. **WHEP, Caudal loses:** 87 % of one core at 100 viewers; at 300 only 245 Mbps egress vs 1,377 and UDP drops (the single-task WebRTC engine, `crates/caudal-webrtc/src/lib.rs:89`, now measured) → WebRTC across cores moves up from "later". **Latency** (stamp to decoded frame): LL-HLS 379 vs 234 ms median (MediaMTX better; PART-HOLD-BACK 0.6 vs 0.5 s); RTSP 177 vs 275 ms (Caudal better, p95 381).
**Queue after PR #1:** (1) Firefox WHEP regression: real Firefox gets readyState 3 but `videoWidth` 0 (Chromium/WebKit OK); last green `68b4e2b`; candidates `85d0370`, `9949eda`, `19daa98` → bisect with `tests/browser` (`npx playwright test --project firefox tests/whep.spec.ts`). (2) HLS viewers across a publisher reconnect (audit: new packager writes ENDLIST, `crates/caudal-hls/src/lib.rs:148`). (3) Rest of `docs/research/TEST-AUDIT.md` Phase 1: fuzz targets for parsers (cargo-fuzz installed), independent oracles (VLC, GStreamer, tshark, TSDuck, threefive, standardwebhooks — all installed), SRT amplification capture, 7 port helpers that still probe-and-release. (4) `docs/research/BLINDSPOTS.md` items: musl vs mimalloc long-run RSS, batched UDP I/O.
