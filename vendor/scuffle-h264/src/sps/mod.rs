...
impl Sps {
    ...
    pub fn height(&self) -> u64 {
        // Caudal patch: every field feeding this is exp-Golomb-decoded
        // straight from the SPS bits, with no relationship enforced between
        // `pic_height_in_map_units_minus1` and the crop offsets -- an SPS
        // can legally parse (this function only runs once parsing already
        // succeeded) while claiming crop offsets larger than the frame
        // itself. The plain `-`/`+`/`*` below then panicked (subtract with
        // overflow) or, in a release build (no overflow checks), silently
        // wrapped to a bogus multi-exabyte "height". Found via
        // `caudal-rtmp`'s `video_dimensions` by
        // `fuzz/fuzz_targets/rtmp_flv_amf.rs`. Saturating arithmetic keeps
        // every valid SPS's answer identical and turns every other input
        // into 0 instead of a panic or a wrapped garbage value.
        ...
        self.frame_crop_info.as_ref().map_or(base_height, |crop| {
            base_height.saturating_sub(crop.frame_crop_top_offset.saturating_add(crop.frame_crop_bottom_offset).saturating_mul(2))
        })
    }
    ...
    pub fn width(&self) -> u64 {
        // Caudal patch: see `height`'s comment just above, same issue, same fix.
        let base_width = self.pic_width_in_mbs_minus1.saturating_add(1).saturating_mul(16);

        self.frame_crop_info.as_ref().map_or(base_width, |crop| {
            base_width.saturating_sub(crop.frame_crop_left_offset.saturating_add(crop.frame_crop_right_offset).saturating_mul(2))
        })
    }
...
mod tests {
    ...
    /// Caudal patch regression: found via `caudal-rtmp`'s `video_dimensions`
    /// (which calls `width()`/`height()` right after a successful parse) by
    /// `fuzz/fuzz_targets/rtmp_flv_amf.rs`. Nothing in `Sps::parse` checks
    /// that the crop offsets relate sanely to the frame size, so a
    /// perfectly parseable SPS can claim crop offsets far larger than the
    /// frame itself; `width()`/`height()` used to compute
    /// `base - crop_offsets * 2` with plain arithmetic, panicking (subtract
    /// with overflow) in any debug build -- which includes every `cargo
    /// test`/`nextest` run and this fuzz target, though not a release
    /// build, where the same bug would silently produce a wildly wrong
    /// "width" instead.
    #[test]
    fn width_and_height_saturate_on_crop_offsets_bigger_than_the_frame() {
        let mut sps = Vec::new();
        let mut writer = BitWriter::new(&mut sps);

        writer.write_bit(false).unwrap(); // forbidden zero bit
        writer.write_bits(0, 2).unwrap(); // nal_ref_idc
        writer.write_bits(7, 5).unwrap(); // nal_unit_type = SPS

        writer.write_bits(77, 8).unwrap(); // profile_idc = 77: no sps_ext
        ...
        // pic_width_in_mbs_minus1 = 0 -> base_width = (0 + 1) * 16 = 16
        writer.write_exp_golomb(0).unwrap();
        // pic_height_in_map_units_minus1 = 0, frame_mbs_only_flag (below) =
        // true -> base_height = (2 - 1) * (0 + 1) * 16 = 16
        writer.write_exp_golomb(0).unwrap();

        writer.write_bit(true).unwrap(); // frame_mbs_only_flag
        writer.write_bit(false).unwrap(); // direct_8x8_inference_flag
        writer.write_bit(true).unwrap(); // frame_cropping_flag

        // Every crop offset far exceeds the 16x16 frame computed above.
        writer.write_exp_golomb(10_000).unwrap(); // frame_crop_left_offset
        writer.write_exp_golomb(10_000).unwrap(); // frame_crop_right_offset
        writer.write_exp_golomb(10_000).unwrap(); // frame_crop_top_offset
        writer.write_exp_golomb(10_000).unwrap(); // frame_crop_bottom_offset

        writer.write_bit(false).unwrap(); // vui_parameters_present_flag
        writer.finish().unwrap();

        let result = Sps::parse(std::io::Cursor::new(&sps)).unwrap();
        assert_eq!(result.width(), 0, "must not panic; saturates instead of underflowing");
