...
/// Caudal patch: upper bound on the pre-allocation hint `decode_object`'s
/// `EcmaArray` branch and `decode_strict_array` take from a wire-controlled
/// 4-byte count, before either has read a single element. See both call
/// sites below.
    ...
    R: ZeroCopyReader<'a>,
{
    ...
    pub fn decode_object(&mut self) -> Result<Amf0Object<'a>, Amf0Error> {
        ...
        match header {
            ...
            ObjectHeader::EcmaArray { size } => {
                // Caudal patch: `size` is a 4-byte, wire-controlled count
                // (up to u32::MAX) taken on faith as a capacity hint. Every
                // real AMF0 message (RTMP `onMetaData`/`onCuePoint`, the
                // only ECMA arrays Caudal ever decodes) has at most a
                // handful of fields, so a 4-byte value from any RTMP
                // publisher used to be able to make this pre-allocate
                // gigabytes before reading a single byte of the array
                // itself -- and a failed allocation aborts the whole
                // process (Rust's default allocator does not panic, it
                // aborts), taking down every stream, not just this
                // connection. Capping the *hint* changes nothing about
                // correctness: a legitimately large array (still capped by
                // `MAX_PARTIAL_CHUNK_SIZE`/message-length limits upstream
                // long before it could reach millions of entries) just
                // reallocates a few more times while filling in below.
                // Found by `fuzz/fuzz_targets/rtmp_flv_amf.rs`.
                let mut object = Amf0Object::with_capacity((size as usize).min(MAX_PREALLOC_HINT));
    ...
    pub fn decode_strict_array(&mut self) -> Result<Amf0Array<'a>, Amf0Error> {
        ...
        // Caudal patch: same unchecked-capacity-hint issue as the
        // `EcmaArray` branch of `decode_object` above, same fix. Found by
        // `fuzz/fuzz_targets/rtmp_flv_amf.rs`.
        let mut array = Vec::with_capacity(size.min(MAX_PREALLOC_HINT));
...
mod tests {
    ...
    /// Caudal patch regression: found by `fuzz/fuzz_targets/rtmp_flv_amf.rs`.
    /// An `EcmaArray`'s 4-byte element count is taken straight off the
    /// wire; before this patch it was used unclamped as a
    /// `HashMap::with_capacity` hint, so a handful of bytes from any RTMP
    /// publisher (`onMetaData`/`onCuePoint` are both AMF0 ECMA arrays)
    /// could make the decoder try to pre-allocate tens of gigabytes before
    /// reading a single element -- and a failed allocation aborts the
    /// whole process (Rust's global allocator calls `handle_alloc_error`,
    /// which is not a catchable panic), not just this connection. A tiny
    /// buffer claiming `u32::MAX` elements must instead fail normally
    /// (running out of bytes partway through the loop) without attempting
    /// that allocation.
    #[test]
    fn a_huge_ecma_array_size_does_not_preallocate_it() {
        #[rustfmt::skip]
        let bytes = [
            Amf0Marker::EcmaArray as u8,
            0xFF, 0xFF, 0xFF, 0xFF, // size = u32::MAX
            0, 3, b'a', b'b', b'c', // one real key
            Amf0Marker::Boolean as u8,
            1, // one real value, then the buffer ends
        ];

        let mut decoder = Amf0Decoder::from_slice(&bytes);
        // Must return an error (ran out of bytes) quickly, not hang or
        // abort trying to allocate ~64 exabytes of hash map capacity.
        assert!(decoder.decode_object().is_err());
    }

    /// Same issue, `StrictArray`'s `Vec::with_capacity`.
    #[test]
    fn a_huge_strict_array_size_does_not_preallocate_it() {
        #[rustfmt::skip]
        let bytes = [
            Amf0Marker::StrictArray as u8,
            0xFF, 0xFF, 0xFF, 0xFF, // size = u32::MAX
        ...
        let mut decoder = Amf0Decoder::from_slice(&bytes);
        assert!(decoder.decode_strict_array().is_err());
    }
}
