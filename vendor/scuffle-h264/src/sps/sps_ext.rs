            ...
            for i in 0..count {
                ...
                if bit {
                    ...
                    for _ in 0..size {
                        ...
                        // Caudal patch: `delta_scale` comes straight from
                        // `read_signed_exp_golomb`, which (after its own
                        // patch, see vendor/README.md) can still return
                        // anything in `i64`'s full range for a bitstream
                        // that doesn't look like a real scaling matrix.
                        // `next_scale` is otherwise always in `[0, 256)`
                        // here, but this file's own comment says none of
                        // it is used for decoding -- only the `== 0` check
                        // below, to decide when to stop -- so saturating
                        // and normalizing into `[0, 256)` changes nothing
                        // for any real encoder while never overflowing.
                        // Found (a different fuzzer-mutated overflow along
                        // this exact call chain) by
                        // `fuzz/fuzz_targets/rtmp_flv_amf.rs`.
                        next_scale = next_scale.saturating_add(delta_scale).saturating_add(256).rem_euclid(256);
                        if next_scale == 0 {
                            break;
                        }
                    }
...
mod tests {
    ...
    /// through `caudal-rtmp`'s `video_dimensions` -> `AVCDecoderConfigurationRecord`
    /// -> here. `write_signed_exp_golomb`/`read_signed_exp_golomb` can
    /// legitimately round-trip values across all of `i64` (see
    /// `scuffle-expgolomb`'s own patch), but this loop's `next_scale =
    /// next_scale + delta_scale + 256` used plain arithmetic: a single
    /// `delta_scale` near `i64::MAX` (or `i64::MIN`) overflowed it and
    /// panicked, on the very first entry of the very first scaling list.
    #[test]
    fn a_delta_scale_near_i64_max_does_not_overflow_next_scale() {
        let mut data = Vec::new();
        let mut writer = BitWriter::new(&mut data);

        writer.write_exp_golomb(0).unwrap(); // chroma_format_idc = 0: 8 lists, no separate_color_plane_flag
        ...
        writer.write_bit(true).unwrap(); // seq_scaling_matrix_present_flag

        // list 1 of 8, present.
        writer.write_bit(true).unwrap();
        // next_scale starts at 8; 8 + i64::MAX + 256 overflowed and
        // panicked before this patch. Saturating, it becomes
        // i64::MAX.saturating_add(256) = i64::MAX, then rem_euclid(256) =
        // 255 (i64::MAX's low 8 bits are all set).
        writer.write_signed_exp_golomb(i64::MAX).unwrap();
        // Bring next_scale from 255 back to 0 to break out of this list
        // without needing to supply all 16 entries.
        writer.write_signed_exp_golomb(-255).unwrap();

        // lists 2 thru 8: nothing.
        writer.write_bits(0, 7).unwrap();

        writer.finish().unwrap();

        let mut reader = BitReader::new_from_slice(&mut data);
        let sps_ext = SpsExtended::parse(&mut reader).expect("must not panic");
        assert_eq!(sps_ext.scaling_matrix[0], vec![i64::MAX, -255]);
    }
}
