...
impl TsMux {
    ...
    fn write_video(&mut self, info: &TrackInfo, frame: &Frame, pcr_27mhz: Option<u64>) {
        ...
        // pes_packet_len 0: unbounded, the norm for video (see `crate::ts`,
        // which parses it the same way on the way in).
        self.write_pes(VIDEO_PID, header, 0, &payload, frame.keyframe, pcr_27mhz);
    }
