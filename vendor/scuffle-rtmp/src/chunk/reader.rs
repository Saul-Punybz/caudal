...
pub struct ChunkReader {
    ...
    /// Caudal patch: the timestamp delta last seen on each chunk stream.
    /// A Type 3 chunk that starts a new message applies it again (RTMP spec
    /// 5.3.1.2.4; FFmpeg's `rtmppkt.c` does the same). Upstream 0.2.3
    /// reused the previous timestamp unchanged, which froze timestamps for
    /// encoders that send constant-rate frames as Type 3.
...
impl ChunkReader {
    ...
    pub fn read_chunk(&mut self, buffer: &mut BytesMut) -> Result<Option<Chunk>, crate::error::RtmpError> {
        ...
        loop {
            ...
            // Caudal patch: remember the delta a following Type 3 chunk reuses.
    ...
    fn read_message_header(
        ...
        match header.format {
            ...
            ChunkType::Type3 => {
                ...
                // Caudal patch: a Type 3 chunk that starts a new message (no
...
mod tests {
    ...
    /// Caudal patch: Type 3 chunks that start new messages advance the
