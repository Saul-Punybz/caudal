...
- [x] **M9: RTSP** pull (retina) and server (TCP interleaved, UDP unicast, RTSPS).
...
- [x] **M13: 24/7 channels from files** (decided by Saul, 18 Sep 2026: Caudal includes playout, all in Rust, no concern about overlapping ANTENA787). Playlists and schedules of MP4/TS/MKV files published as a continuous live stream, with transitions on keyframes and timestamp stitching; normalization through the transcoder when files differ.
- [ ] **M12: Open Media Transport (OMT), the open NDI alternative.** Pure-Rust port of the MIT reference (libomtnet C#, ~10.6K lines) and the VMX codec (libvmx C, ~23.5K lines incl. SIMD), mDNS discovery with `mdns-sd`. **Correction (21 Sep 2026): not the first native Rust OMT.** `MikanseiLaboratory/openmediatransport-rs` (MIT, ~9K lines, 95 commits, 1 star, listed in upstream's `DOWNLOADS.md`) already implements the protocol over TCP with DNS-SD discovery plus a VMX codec (`vmx-rs`), and claims no interop validation against vMix or OBS. Our differentiator is verification, not firstness: `vmx-codec` is byte-identical to the C++ reference, theirs is unverified. Whether to build on it, contribute to it, cross-check against it or go our own way is an open decision. Send and receive, so Caudal can take in and put out LAN video for live production.
...
| 14 | **Future, not now (Saul, 19 Sep 2026: "solo lo estaba preguntando"). MCP server:** `caudal mcp` exposes the REST API as Model Context Protocol tools so an operator can run Caudal from Claude (Claude Code / Desktop over stdio, or streamable HTTP for claude.ai). Official Rust SDK (`rmcp`); wraps `/api/v1` with an admin API token, no new server internals. Read-only by default (streams, alerts, recordings, schedules, failover and cluster status, `caudal doctor`); write tools (clip, failover switch, channel skip, SCTE-35 cue, restream, config reload) only when enabled, scoped per token. Verified with the MCP Inspector and a real Claude Code session. | Sonnet | future |
| 13 | **Delivery at scale (Saul, 19 Sep 2026):** (1) batched UDP sends (`sendmmsg`/GSO on Linux, Apple batch API) for WebRTC and RTSP/UDP, branch `perf/webrtc-batched-send`; (2) **IP multicast output**: MPEG-TS and RTP over UDP multicast (and RTSP multicast) for cable/IPTV, campus and hotel networks, where one copy leaves the server and the switches replicate it (MistServer has it; the CAtv market needs it); (3) HLS over HTTPS with kTLS + `sendfile` on Linux (kernel encrypts and sends segments without copying through Caudal); (4) evaluate `io_uring` for the HTTP/UDP hot paths and send pacing (`fq` qdisc / `SO_TXTIME`) against burst loss. Each measured with `bench/` before and after; kernel bypass (DPDK/AF_XDP) stays out of scope. | Opus | planned |
