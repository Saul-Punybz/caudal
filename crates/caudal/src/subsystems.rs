...
impl Supervisor {
    ...
    pub fn start(cfg: &Config, registry: Arc<Registry>) -> Started {
        ...
        let failover = caudal_failover::start(
            registry.clone(),
            caudal_failover::FailoverConfig { entries: cfg.failovers().expect("validated"), buffer },
        );
        ...
        Started { supervisor, restream_router, channel_router, failover_router, failover }
    }
