...
/// Caudal patch: fast IMDCT (O(N log N)), numerically matching the direct
/// [`imdct`] (kept as the oracle). The direct form costs `N·N/2` `f64` cosines
/// per block (2.1 M for a long block): ~6 ms per 1024-sample frame on an Apple
/// M4 and slower than real time on a 4-vCPU x86 CI runner, so a live decoder
/// fell behind its input. Method: the IMDCT is a DCT-IV of the `L = N/2`
/// coefficients, unfolded by its symmetries; the DCT-IV runs through the same
/// pack/pre-rotation, `N/4`-point FFT and post-rotation as [`mdct_fast`] (which
/// is `2·DCT-IV` of the folded input), with the same cached twiddles.
pub fn imdct_fast(spec: &[f32]) -> Vec<f32> {
    ...
    // DCT-IV: y[j] = Σ_k X[k]·cos(π/L·(j+½)(k+½)).
    let (mut re, mut im) = (vec![0f64; m], vec![0f64; m]);
    for p in 0..m {
        let (xr, xi) = (spec[2 * p] as f64, spec[l - 1 - 2 * p] as f64);
        re[p] = xr * tw.pre_c[p] + xi * tw.pre_s[p];
        im[p] = xi * tw.pre_c[p] - xr * tw.pre_s[p];
    }
    fft(&mut re, &mut im, &tw.fft_c, &tw.fft_s);
    let mut y = vec![0f64; l];
    for p in 0..m {
        let (vr, vi) = (re[p], im[p]);
        y[2 * p] = vr * tw.post_c[p] + vi * tw.post_s[p];
        y[l - 1 - 2 * p] = vr * tw.post_s[p] - vi * tw.post_c[p];
    }
    // out[i] = (2/N)·y'[i + L/2], where y' extends y by the kernel's symmetries:
    // y'[j] = -y[2L-1-j] on [L, 2L) and y'[j] = -y[j-2L] on [2L, 3L).
    let scale = 2.0 / n as f64;
    (0..n)
        .map(|i| {
            let j = i + l / 2;
...
mod tests {
    ...
    /// Caudal patch: the fast IMDCT matches the direct oracle at both AAC block
    /// sizes (and a non-cached size), on raw and decoder-scale coefficients.
    #[test]
    fn imdct_fast_matches_direct() {
        for half in [1024usize, 128, 32] {
            for amp in [1.0f64, 32768.0] {
                let spec: Vec<f32> = (0..half)
                    .map(|k| {
                        (amp * (0.5 * (k as f64 * 0.37).sin() + 0.3 * (k as f64 * 0.011).cos())
                            / (1.0 + k as f64 * 0.01)) as f32
                    })
                    .collect();
                let (a, b) = (imdct(&spec), imdct_fast(&spec));
                assert_eq!(a.len(), b.len());
                let peak = a.iter().fold(0f32, |m, v| m.max(v.abs()));
                let err = a.iter().zip(&b).fold(0f32, |m, (x, y)| m.max((x - y).abs()));
                assert!(err <= peak * 1e-5, "N/2={half} amp={amp}: max err {err} vs peak {peak}");
            }
        }
    }
