...
**Last updated:** 19 Sep 2026, ~02:10 UTC (benchmark vs MediaMTX merged; PR #1 open, CI running; see RESUME HERE)

## What it is
Open-source rewrite of MistServer in Rust. Full plan and evidence in `PLAN.md`; reuse inventory in `REUSE.md`.

## RESUME HERE (19 Sep 2026, ~01:35 UTC) — read this first in a new session
**Checkout:** `main` is clean and pushed. Open work is on branch **`harden/ci-and-crash-safety`** = **PR #1** (https://github.com/Saul-Punybz/caudal/pull/1), 4 commits, compiled and tested by GitHub CI only (the Mac was busy with the benchmark):
1. `024ec73` release `panic = "unwind"` (+ contract test `crates/caudal/tests/release_profile.rs`); RTMP/RTSP/RTSPS/SRT listeners retry on accept errors; health integration test lost-wake-up fix (was the Linux CI failure); CI installs ffmpeg 8.1 (BtbN) + srt-tools, fails on any SKIP except `mediastreamvalidator`.
2. `2b040e5` RTMP 49.7-day wrap: `RtmpClock` in `crates/caudal-rtmp/src/demux.rs`, one per stream in `session.rs`, unit tests.
3. `8535be1` `[hls] cue_out_tags` (default off): legacy `EXT-X-CUE-OUT` / `-CONT` / `EXT-X-CUE-IN`, test `legacy_cue_out_tags_mark_the_break`.
4. `d491baa` CI on **cargo-nextest** (`.config/nextest.toml` profile `ci`: a test is killed after 4 min and named) because the `test` step hung 34+ min with no log. Workflow `concurrency` cancels superseded runs.
**First thing to do:** `gh run list -b harden/ci-and-crash-safety -L 3` → read the CI result of `d491baa`. If a test was TERMINATED/TIMEOUT, that is the hang to fix. If the SKIP gate fails, install what's missing or fix the test. **None of PR #1 has been compiled locally**: run the full local gate on the branch before merging (`cargo fmt --all --check`, clippy `-D warnings`, `cargo nextest run` or `cargo test --workspace --no-fail-fast`, `CAUDAL_E2E=1 cargo test -p caudal -- --test-threads=1`, `cargo deny check`). Merge only when GitHub CI is green.
**The machine is free** (benchmark finished and merged); local compiling is fine again.
**Benchmark vs MediaMTX v1.21.0 — DONE, merged (`f87cdf7`): report `docs/research/BENCH-MEDIAMTX.md`, rerun with `bench/run.sh` (~80 min), load client `bench/client` (own workspace).** Apple M4, 10 cores, 3 runs each; other apps used 1.3–3.3 cores during the runs, so compare the two servers, don't quote absolutes (rerun on an idle machine). Caudal wins: binary 21.8 vs 54.1 MB (over the plan's 10–20 MB estimate), idle RSS 14 vs 35 MB, **LL-HLS fan-out** CPU 8.7/26/83 vs 27/72/168 % at 100/300/1,000 viewers with less RSS (215 vs 339 MB at 1,000). MediaMTX wins: RSS with one live stream (80 vs 98 MB; 87 with a 14 s buffer), RTSP CPU (~15 % less), **LL-HLS latency at the live edge 234 vs 379 ms** (with hold-back ≈ 0.73 vs 0.98 s estimated in a player). RTSP latency: tie (177 vs 275 ms, noise larger than the gap). RTSP at 1,000: both fail on this laptop's kernel network memory. **WHEP at 300: Caudal 245 vs 1,377 Mbps — a real Caudal bug, and NOT CPU-bound** (47 % of one core, busiest thread 18 %): the kernel dropped 12,700 UDP datagrams on full socket buffers → the single UDP socket's buffers/send path stall (`crates/caudal-webrtc/src/engine.rs`); at 1,000 sessions time out. Fix candidates, in order: larger SO_SNDBUF/SO_RCVBUF, never block the recv loop on sends, batched sends (sendmmsg/GSO), then shard across sockets/cores. Not measured: WHEP latency (no headless decoder), person-visible latency, RTSP/WHEP at 1,000 with a second load machine.
**Queue after PR #1:** (0) **WHEP throughput bug** from the benchmark (see above; verify with `bench/run.sh` WHEP cells). (1) Firefox WHEP regression: real Firefox gets readyState 3 but `videoWidth` 0 (Chromium/WebKit OK); last green `68b4e2b`; candidates `85d0370`, `9949eda`, `19daa98` → bisect with `tests/browser` (`npx playwright test --project firefox tests/whep.spec.ts`). (2) HLS viewers across a publisher reconnect (audit: new packager writes ENDLIST, `crates/caudal-hls/src/lib.rs:148`). (3) Rest of `docs/research/TEST-AUDIT.md` Phase 1: fuzz targets for parsers (cargo-fuzz installed), independent oracles (VLC, GStreamer, tshark, TSDuck, threefive, standardwebhooks — all installed), SRT amplification capture, 7 port helpers that still probe-and-release. (4) `docs/research/BLINDSPOTS.md` items: musl vs mimalloc long-run RSS, batched UDP I/O.
**Later (Saul: good to have):** GPU transcoding `[transcode] hw = "auto"`, WebRTC across cores (PLAN.md "C. Later").
**Rule from Saul:** verify with tools outside Claude (memory `saul-verificacion-externa`); say what is not verified.
**Latency in real browsers (GitHub runner, 19 Sep):** LL-HLS steady state Chromium 1.29 s, WebKit 1.33 s, Firefox 1.44 s.
**Repos public (19 Sep, Saul):** `Saul-Punybz/caudal`, `Saul-Punybz/open-media-transport` (Actions free now; CI had silently not run for a day because of a billing block).
**Wave 1+2 merged:** SCTE-35, UI screens, RTSP UDP/RTSPS, admin login, health alerts, hot reload (details below and in git log). MSRV 1.95.
**Open items:** SCTE-35 (API can't ask for splice_insert; onCuePoint layouts unverified; cues on DTS not PTS; cue before first keyframe dropped; channel-file cues ignored; no real SSAI test). Admin (no real IdP tested; Entra needs `allowed_groups`; behind a proxy the rate limit sees the proxy IP; sessions in memory; Docker image refuses to start without `[admin]` — Saul to decide, recommended: keep + document). Health (`publisher_lost` map not pruned for one-off names). Not verified: RTSP with VLC, UI Skip on a real channel, Restreams screen with a live target, login screen in a browser, Opus-in-HLS on a real Apple device.
Decisions: history rewrite declined (87 MB stay). scuffle-rtmp bug reported upstream: https://github.com/ScuffleCloud/scuffle/issues/650.
