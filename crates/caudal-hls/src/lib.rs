...
pub fn router(registry: Arc<Registry>, cfg: HlsConfig) -> axum::Router {
    ...
    Router::new()
        .route("/hls/{name}/{file}", get(hls_file))
        .route("/play/{name}", get(play))
        .with_state(hls)
}
...
fn respond(status: StatusCode, content_type: &'static str, cache: &'static str, body: impl Into<axum::body::Body>) -> Response {
...
async fn hls_file(State(hls): State<Arc<Hls>>, Path((name, file)): Path<(String, String)>, RawQuery(query): RawQuery) -> Response {
