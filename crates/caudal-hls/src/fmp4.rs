...
use mp4_atom::{
    Atom, Audio, Avc1, Avcc, Dinf, Dref, Encode, Esds, Ftyp, Hdlr, Hvc1, Hvcc, Mdhd, Mdia, Minf, Moof, Moov,
    Mp4a, Mfhd, Mvex, Mvhd, Smhd, Stbl, Stco, Stsd, Tfdt, Tfhd, Tkhd, Traf, Trak, Trex, Trun, TrunEntry, Url,
    Visual, Vmhd, esds,
};
...
fn trak(t: &Mp4Track) -> Option<Trak> {
    ...
    let (width, height) = info.video.map(|v| (v.width.min(u16::MAX as u32) as u16, v.height.min(u16::MAX as u32) as u16)).unwrap_or((0, 0));
                ...
                esds: Esds {
                    es_desc: esds::EsDescriptor {
                        ...
                        dec_config: esds::DecoderConfig {
                            ...
                            stream_type: 5,
                            up_stream: 0,
                            dec_specific: Some(esds::DecoderSpecific { profile, freq_index, chan_conf, raw: asc.to_vec() }),
                            ..Default::default()
                        },
