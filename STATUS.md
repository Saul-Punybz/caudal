...
**Next:** benchmark vs MediaMTX v1.21 (Opus) on a quiet machine: no agents compiling (running). Then external verification (VLC, GStreamer, tshark, TSDuck, threefive, standardwebhooks, cargo-fuzz, cargo-mutants; GitHub CI now runs: repo public since 19 Sep). Then the work from `docs/research/BLINDSPOTS.md` (Gemini review checked by hand): **RTMP timestamp wrap at 49.7 days (bug)**, legacy `EXT-X-CUE-OUT/IN`, SRT amplification capture, musl vs mimalloc long-run RSS, batched UDP I/O.
...
**Test audit (19 Sep, Opus, read-only; `docs/research/TEST-AUDIT.md`), claims checked by hand:** (confirmed) release `panic = "abort"` turns any task panic into a full server crash, untestable in debug; CI's `cargo test --workspace` stops at the first failing crate, so e2e never ran on Linux; RTMP/RTSP listeners die on one `accept()` error (`caudal-rtmp/src/lib.rs:37`, `caudal-rtsp/src/server.rs:218,248`); SRT tests SKIP in CI (no `srt-live-transmit`). (plausible, needs a run) HLS viewers dropped on publisher reconnect; S3 over HTTPS from the `FROM scratch` image; publish without `[auth]` on public binds.
**Work queue once the benchmark ends (machine free):** (1) compile + verify the health test lost-wakeup fix (uncommitted in `crates/caudal-health/tests/integration.rs`); (2) `panic = "unwind"` in release + a test that a panicking task doesn't kill the server; (3) CI: `--no-fail-fast`, install srt-tools, fail on SKIP, run all `caudal` tests with `CAUDAL_E2E=1`; (4) listeners survive accept errors; (5) Firefox WHEP bisect; (6) RTMP 49.7-day wrap; (7) HLS across publisher reconnect; (8) CUE-OUT; (9) audit Phase 1 rest (fuzzing, independent oracles).
**Found by GitHub CI (first runs since the billing block, 19 Sep):** (1) `cargo fmt --check` had been failing before any Linux test ran: fixed (`9bb6c91`); local gate now includes fmt. (2) **Firefox WHEP regression:** real Firefox reaches readyState 3 but `videoWidth` stays 0 (no decoded video); Chromium and WebKit pass. Last green browser run `68b4e2b` (18 Sep 12:32, Firefox included). Candidates since: `85d0370` (WHIP/WHEP on str0m), `9949eda` (Cue arm), `19daa98` (clippy collapse in `engine.rs`). Bisect after the benchmark.
