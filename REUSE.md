...
| Speech-to-text (live captions) | huggingface/candle (`candle-core`/`-nn`/`-transformers`, Whisper) | 0.9.2 | MIT OR Apache-2.0 | — | 2026 | depend (19 Sep 2026); pinned below 0.10, which pulls `tokenizers` + Oniguruma (C). Weights: OpenAI Whisper, MIT, fetched by the operator. whisper-rs (whisper.cpp, C++) not needed: candle keeps up (docs/research/CAPTIONS.md) |
| AAC-LC decode (captions) | Remade-With-Rust `rusty_aac` | 0.5.0 | Apache-2.0 | — | 2026 | depend (19 Sep 2026) instead of Symphonia (MPL-2.0, excluded by deny.toml) |
| Opus decode (captions) | TadeuszWolfGang/Rusopus (`opus-decoder`) | 0.1.1 | MIT OR Apache-2.0 | — | 2026 | depend (19 Sep 2026); no unsafe, decodes straight to 16 kHz mono |
